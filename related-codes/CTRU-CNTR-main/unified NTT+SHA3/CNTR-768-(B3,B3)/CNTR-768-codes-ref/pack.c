...
void unpack_sk(poly *r, const unsigned char *a)
{
    ...
    for (i = 0; i < CTRU_N / 8; ++i)
    {
        ...
        r->coeffs[8 * i + 1] = ((a[5 * i + 0] >> 5) | ((uint16_t)a[5 * i + 1] << 3)) & 0x1f;
        r->coeffs[8 * i + 2] = (a[5 * i + 1] >> 2) & 0x1f;
        r->coeffs[8 * i + 3] = ((a[5 * i + 1] >> 7) | ((uint16_t)a[5 * i + 2] << 1)) & 0x1f;
        r->coeffs[8 * i + 4] = ((a[5 * i + 2] >> 4) | ((uint16_t)a[5 * i + 3] << 4)) & 0x1f;
        r->coeffs[8 * i + 5] = (a[5 * i + 3] >> 1) & 0x1f;
        r->coeffs[8 * i + 6] = ((a[5 * i + 3] >> 6) | ((uint16_t)a[5 * i + 4] << 2)) & 0x1f;
