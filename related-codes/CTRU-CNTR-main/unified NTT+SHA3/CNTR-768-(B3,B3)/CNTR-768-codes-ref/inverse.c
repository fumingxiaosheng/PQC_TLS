...
{
    int16_t Phi[CTRU_MATRIX_N + 1], F[CTRU_MATRIX_N + 1], V[CTRU_MATRIX_N + 1], S[CTRU_MATRIX_N + 1];
    ...
    for (i = 0; i < CTRU_MATRIX_N; ++i)
        Phi[i] = 0;
    Phi[0] = 1;
    Phi[CTRU_MATRIX_N] = -zeta;

    for (i = 0; i < CTRU_MATRIX_N; ++i)
        F[CTRU_MATRIX_N - 1 - i] = f[i];
    F[CTRU_MATRIX_N] = 0;

    Delta = 1;

    for (i = 0; i < CTRU_MATRIX_N + 1; ++i)
        V[i] = 0;
    ...
    for (loop = 0; loop < 2 * CTRU_MATRIX_N - 1; ++loop)
    {
        for (i = CTRU_MATRIX_N; i > 0; --i)
            V[i] = V[i - 1];
        V[0] = 0;
    ...
    for (i = 0; i < CTRU_MATRIX_N; ++i)
        finv[i] = fq_freeze(scale * (int32_t)V[CTRU_MATRIX_N - 1 - i]);
